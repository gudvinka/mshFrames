...
function msh:OnEnable()
    ...
    msh:RegisterEvent("UNIT_AURA", function(_, unit)
        for i = 1, 40 do
            local rf = _G["CompactRaidFrame" .. i]
            if rf and rf.displayedUnit == unit and rf.mshLayersCreated then
                msh.UpdateAuras(rf)
            end
        end
        ...
        for i = 1, 5 do
            local pf = _G["CompactPartyFrameMember" .. i]
            if pf and pf.displayedUnit == unit and pf.mshLayersCreated then
                msh.UpdateAuras(pf)
            end
        end
    end)
