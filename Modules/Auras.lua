        ...
        if pool then
            ...
            if not data.enabled then
                ...
                for i = 1, #pool do
                    ...
                    if icon and type(icon) == "table" then
                        local success, isShown = pcall(function() return icon.IsShown and icon:IsShown() end)
                        ...
                        if success and isShown then
                            ...
                            if data.isCustom or (icon == frame.CenterDefensiveBuff or icon == frame.centerStatusIcon) then
                                local currentSize = data.size or 20
                                icon:SetSize(currentSize, currentSize)
                            else
                                if not (isBlizz and pool == frame.debuffFrames) then
